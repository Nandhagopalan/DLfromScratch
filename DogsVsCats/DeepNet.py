# -*- coding: utf-8 -*-
"""
Created on Wed Jan 30 21:10:45 2019
...
'''
networkArchitecture = [
    {"input_dim": X_train.shape[0], "output_dim": 100, "activation": "relu"},
    {"input_dim": 100, "output_dim": 50, "activation": "relu"},
    {"input_dim": 50, "output_dim": 60, "activation": "relu"},
    {"input_dim": 60, "output_dim": 75, "activation": "relu"},
    {"input_dim": 75, "output_dim": 50, "activation": "relu"},
...
def single_layer_forward_propagation(A_prev, W_curr, b_curr, activation="relu"):
    ...
    if activation is "relu":
        activation_func = relu(Z_curr)
    elif activation is "sigmoid":
        activation_func = sigmoid(Z_curr)
    else:
...
def single_layer_backward_propagation(dA_curr, W_curr, b_curr, Z_curr, A_prev, activation="relu"):
    ...
    # selection of activation function
    if activation is "relu":
        backward_activation_func = relu_backward(dA_curr, Z_curr)
    elif activation is "sigmoid":
        backward_activation_func = sigmoid_backward(dA_curr, Z_curr)
    else:
        raise Exception('Non-supported activation function')
    
    # calculation of the activation function derivative
    dZ_curr = backward_activation_func
...
'''
Updating the params using gradient descent
'''
...
def train(X, Y, nn_architecture, epochs, learning_rate, verbose=True, callback=None):
    ...
    for i in range(epochs):
        ...
        accuracy = get_accuracy_value(Y,Y_hat)
...
# Training
params_values = train(X_train,y_train, networkArchitecture, 2000, 0.01)


# Prediction
Y_test_hat, _ = full_forward_propagation(X_test, params_values, networkArchitecture)
